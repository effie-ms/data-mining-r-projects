...
majority_vote <- function(k, points){
  voted_labels <- matrix(c(0),k,2)
  for(j in 1:k){
    voted_labels[j, 1] <- j
...
my_knn_classify <- function(known_data, k, new_point, dist_func){
  ...
  for(i in 1:n){
    ...
    points_with_distances[i, d+1] <- my_dist_func(known_data[i, 1:2], new_point,dist_func, known_data[,1:d])
  }
  ...
  voted_labels <- majority_vote(2, k_points_with_min_dist)
  sorted_voted_labels <-  voted_labels[order(voted_labels[,2], decreasing = TRUE),]
  label <- sorted_voted_labels[1,1]
  return(label)
}

my_knn <- function(x_train, x_valid, k, dist_func)
{
  n <- nrow(x_train) #the number of labeled points
  d <- ncol(x_valid) #the number of features
  x_valid <- cbind(x_valid, c(0))
  for(i in 1:nrow(x_valid)){
    res <- my_knn_classify(x_train, k, x_valid[i,1:d], dist_func)
    x_valid[i,d+1] <- res
  }
  return(x_valid)
}
